<#
  ...
  .DESCRIPTION
    ...
    stream for automating router and switch configurations.
    
    It can be used for any purpose either as a generic telnet client or to access
    the text stream.
  .PARAMETER TelnetHost
    The address of the server or device hosting the telnet service.

  .PARAMETER Port
    The TCP port number of the Telnet service running on the telnet host.
#>
function Connect-GCTelnet {
  Param (
  [Parameter(Mandatory)]
  [String]
  $TelnetHost,
